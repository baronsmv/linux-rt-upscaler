 ...
 * @param kwds   Keyword arguments (cbv, srv, uav, samplers, push_size,
 * bindless).
...
PyObject *vk_Compute_dispatch(vk_Compute *self, PyObject *args) {
  ...
  bool ok = vk_execute_one_time_commands(dev, [&](VkCommandBuffer cmd) {
    ...
    // Make writes available to future command buffers (especially the copy in
    // present)
    ...
    vkCmdPipelineBarrier(
        ...
        VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, // All subsequent commands see the
                                              // writes
...
PyObject *vk_Compute_dispatch_sequence(vk_Compute *self, PyObject *args,
  ...
  if (copy_src_obj != Py_None && copy_dst_obj != Py_None) {
    ...
    if (!src_buf->buffer || !dst_img->image) {
      PyErr_SetString(PyExc_TypeError,
                      "copy_src must be buffer, copy_dst texture");
      return nullptr;
    }
