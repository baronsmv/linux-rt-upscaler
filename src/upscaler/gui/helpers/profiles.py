...
class ProfileActions:
    ...
    def select_profile(self, name: str) -> None:
        ...
        if name == (self._cm.active_profile_name or ""):
            return
        if not self.maybe_save_before_switch():
            return
        self._cm.set_active_profile(name)
    ...
    def add_profile(self) -> None:
        ...
        try:
            dlg = ProfileDialog(
                self._mw.gui_config, profiles=self._cm.profiles, parent=self._mw
            )
            if dlg.exec() == QDialog.Accepted:
                name = dlg.profile_name()
                match = dlg.match_criteria()
                self._cm.add_profile(name, match)
    ...
    def edit_profile(self, name: str) -> None:
        ...
        try:
            ...
            if dlg.exec() == QDialog.Accepted:
                ...
                if new_name != name:
                    if new_name in self._cm.profiles:
                        QMessageBox.warning(
                            self._mw,
                            "Duplicate name",
                        ...
                        return
                    self._cm.rename_profile(name, new_name)
                    self._rename_icon_file(name, new_name)
                    self._cm.update_profile_match(new_name, new_match)
                else:
                    self._cm.update_profile_match(name, new_match)

                icon = dlg.get_captured_icon()
                if icon:
                    self._save_icon(new_name if new_name != name else name, icon)

                if self._cm.active_profile_name == new_name:
            ...
            QMessageBox.critical(self._mw, "Error", "Could not edit profile.")
    ...
    def delete_profile(self, name: str) -> None:
        ...
        try:
            ...
            if reply == QMessageBox.Yes:
                ...
                self._cm.delete_profile(name)
                self._sidebar.update_profiles(self._cm.profiles)
                self._sidebar.populate_list(active_name=None)
                if self._cm.active_profile_name == name:
                    self._cm.set_active_profile(None)
        except Exception:
            logger.exception("Failed to delete profile")
            QMessageBox.critical(self._mw, "Error", "Could not delete profile.")

    def move_up(self, name: str) -> None:
        try:
            self._cm.move_profile_up(name)
            self._sidebar.update_profiles(self._cm.profiles)
            self._sidebar.populate_list(active_name=name)
        except Exception:
            logger.exception("Failed to move profile up")
            QMessageBox.critical(self._mw, "Error", "Could not reorder profiles.")

    def move_down(self, name: str) -> None:
        try:
            self._cm.move_profile_down(name)
            self._sidebar.update_profiles(self._cm.profiles)
            self._sidebar.populate_list(active_name=name)
        except Exception:
    ...
    def _save_icon(self, profile_name: str, image: QImage) -> None:
        ...
        image.save(path, "PNG")
        self._cm.set_profile_icon(profile_name, path)
    ...
    def _remove_icon_file(self, profile_name: str) -> None:
        ...
        try:
            ...
            pass
        self._cm.remove_profile_icon(profile_name)
    ...
    def _rename_icon_file(self, old_name: str, new_name: str) -> None:
        ...
        if os.path.isfile(old_path):
            try:
                ...
                self._cm.update_profile_icon_path(new_name, new_path)
            except OSError:
                self._cm.remove_profile_icon(old_name)
                try:
                    os.remove(old_path)
                except OSError:
                    pass
        else:
            self._cm.remove_profile_icon(new_name)
