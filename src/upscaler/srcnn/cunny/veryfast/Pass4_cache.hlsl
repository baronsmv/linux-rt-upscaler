...
void main(uint3 id : SV_DispatchThreadID)
{
    ...
    if (localOutBase.x < maxOut.x && localOutBase.y < maxOut.y)
        OUTPUT[uint3(localOutBase + int2(0, 0), tileParams.outputLayer)] = float4(mul(YR, float3(saturate(yuv.r + r0.x), yuv.yz)), 1.0);
    yuv = mul(RY, INPUT.SampleLevel(SL, float3(fpos + float2(1.0, 0.0) * full_opt, 0), 0).rgb);
    if (localOutBase.x + 1 < maxOut.x && localOutBase.y < maxOut.y)
        OUTPUT[uint3(localOutBase + int2(1, 0), tileParams.outputLayer)] = float4(mul(YR, float3(saturate(yuv.r + r0.y), yuv.yz)), 1.0);
    yuv = mul(RY, INPUT.SampleLevel(SL, float3(fpos + float2(0.0, 1.0) * full_opt, 0), 0).rgb);
    if (localOutBase.x < maxOut.x && localOutBase.y + 1 < maxOut.y)
        OUTPUT[uint3(localOutBase + int2(0, 1), tileParams.outputLayer)] = float4(mul(YR, float3(saturate(yuv.r + r0.z), yuv.yz)), 1.0);
    yuv = mul(RY, INPUT.SampleLevel(SL, float3(fpos + float2(1.0, 1.0) * full_opt, 0), 0).rgb);
    if (localOutBase.x + 1 < maxOut.x && localOutBase.y + 1 < maxOut.y)
        OUTPUT[uint3(localOutBase + int2(1, 1), tileParams.outputLayer)] = float4(mul(YR, float3(saturate(yuv.r + r0.w), yuv.yz)), 1.0);
}