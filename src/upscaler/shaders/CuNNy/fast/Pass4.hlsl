...
void main(uint3 id : SV_DispatchThreadID)
{
    ...
    float2 opt = GetOutputPt();
    float2 base = (gxy + 0.5) * opt;
    V3 yuv;

    yuv = mul(RY, INPUT.SampleLevel(SL, base, 0).rgb);
    OUTPUT[gxy + int2(0,0)] = V4(mul(YR, V3(saturate(yuv.r + r0.x), yuv.yz)), 1.0);

    yuv = mul(RY, INPUT.SampleLevel(SL, base + float2(opt.x, 0.0), 0).rgb);
    OUTPUT[gxy + int2(1,0)] = V4(mul(YR, V3(saturate(yuv.r + r0.y), yuv.yz)), 1.0);

    yuv = mul(RY, INPUT.SampleLevel(SL, base + float2(0.0, opt.y), 0).rgb);
    OUTPUT[gxy + int2(0,1)] = V4(mul(YR, V3(saturate(yuv.r + r0.z), yuv.yz)), 1.0);

    yuv = mul(RY, INPUT.SampleLevel(SL, base + opt, 0).rgb);
    OUTPUT[gxy + int2(1,1)] = V4(mul(YR, V3(saturate(yuv.r + r0.w), yuv.yz)), 1.0);
}
