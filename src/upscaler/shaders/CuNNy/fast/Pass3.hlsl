// CuNNy fast NVL - Pass 3 (12x8)
// Adapted for Compushady compute shader

Texture2D<float4> Input0 : register(t0); // T3
Texture2D<float4> Input1 : register(t1); // T4
Texture2D<float4> Input2 : register(t2); // T5
RWTexture2D<float4> Output0 : register(u0); // T0
RWTexture2D<float4> Output1 : register(u1); // T1
...
#define L2(x, y) Input2.SampleLevel(PointSampler, pos + float2(x, y) * inputPt, 0)
