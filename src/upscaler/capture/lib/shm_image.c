 ...
 * ------------------------------------------------------------------------- */
static void convert_to_bgra(unsigned char *dest, XImage *src,
...
int shm_recreate_if_needed(CaptureContext *ctx) {
    ...
    if (ctx->shminfo.shmid == -1) {
        XDestroyImage(ctx->img); ctx->img = NULL;
        ctx->had_shm_failure = 1;
        return 0;
    }
    ...
    ctx->use_fast_path = (ctx->img->bits_per_pixel == 32 &&
                          ctx->img->format == ZPixmap);
...
int shm_capture_region(CaptureContext *ctx, int rx, int ry, int rw, int rh,
    ...
    if (!shm_recreate_if_needed(ctx)) {
        ...
        x11_lock();
        XImage *fb = XGetImage(ctx->dpy, ctx->xid, ctx->x + rx, ctx->y + ry,
                               rw, rh, AllPlanes, ZPixmap);
        x11_unlock();
        if (!fb) return -1;
        convert_to_bgra(out, fb, 0, 0, rw, rh,
    ...
    if (!ok) {
        ...
        XSync(ctx->dpy, False);
        x11_unlock();
        convert_to_bgra(out, ctx->img, rx, ry, rw, rh,
