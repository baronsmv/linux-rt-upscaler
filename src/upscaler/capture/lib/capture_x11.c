...
int capture_grab_damage(CaptureContext *ctx, unsigned char *output_data,
    ...
    if (ctx->tile_cache) {
        rect_count = tile_cache_detect_changes(ctx, use_full_frame, output_data,
                                               partial_buf, cap_x, cap_y, cap_w, cap_h,
                                               rects, max_rects);

        /* If threshold exceeded, tile_cache_detect_changes already returned a full‑frame rect. */
        if (rect_count == 1 && rects[0].width == ctx->width && rects[0].height == ctx->height) {
            /* Full frame was requested – we may need to recapture if we hadn't already */
            if (!use_full_frame) {
