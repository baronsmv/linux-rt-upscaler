...
unsigned long long tile_compute_hash(unsigned char *buf, int stride,
...
int tile_cache_detect_changes(CaptureContext *ctx,
                              int full_frame,
    ...
    if (full_frame) {
        for (int ty = 0; ty < ctx->tiles_y; ty++) {
            for (int tx = 0; tx < ctx->tiles_x; tx++) {
                ...
                unsigned long long cur_hash = tile_compute_hash(
    ...
    if (changed_count > threshold_tiles) {
        if (ctx->debug) fprintf(stderr, "[capture] threshold exceeded, full frame\n");
        if (!full_frame) {
